...
def PurduePrepBackend(input_str):
    ...
    for url, score in sorted_relevant_urls:
        ...
        if page_questions != None:
            questions.append((page_questions[0], url)) # page variety for demo
            # for question in page_questio)ns:
            #     questions.append((question, url))
        if len(questions) > MAX_NUM_QUESTIONS: return questions
